...
// import { useAddCategory } from '../../../controllers/superAdmin/vendor/category';
import Loader from '../status/Loader'
import SuccessBadge from '../status/SuccessBadge';
import ErrorBadge from '../status/ErrorBadge';
import { useModules, usePostLead } from '../../../controllers/modules';
//=============================================================================
const AddModule = ({ title, description, isOpen, onClose, businessId, refreshModules }) => {
  //=============================================================================
// console.log('businessId===========', businessId);

  const { createModule, isPending, error, isSuccess } = usePostLead();
  ...
  const handleSubmit = async (e) => {
    ...
    moduleData.append('businessId', businessId);
    if (imageFile) {
      moduleData.append('icon', imageFile);
    }

    const { success,successMessage,errorMessage } = await createModule(moduleData);
    ...
    if (success) {
      ...
      console.error('Error adding module:', errorMessage);
    }
  };
      ...
      <div className="animate-slide-down fixed inset-0 flex items-center justify-center bg-[rgba(0,0,0,0.6)] backdrop-blur-sm z-50">
        <div className="bg-background-section p-2 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto transition-transform transform scale-100 border border-accent/60 border-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 p-4">
            <div>
              <label className="block mb-2 text-sm font-medium">Module Name</label>
              <Input
