...
const OnlineClassesTable = ({
  ...
  // console.log('onlineClasses', onlineClasses);
              ...
              >
                ...
                <td className="whitespace-nowrap px-4 py-4">
                  <div className="flex flex-col items-center">
                    {onlineClass.user ? (
                      <img
                        src={onlineClass?.user?.image_url}
                        alt={onlineClass?.user?.name}
                        className="w-10 h-10 rounded-full object-cover mb-1"
                        onError={(e) => {
                          e.target.onerror = null; // Prevent infinite loop on broken image
                          e.target.src = `https://placehold.co/40x40/E0E0E0/757575?text=${getInitials(onlineClass.user?.name)}`; // Fallback to a generic placeholder with initials
                        }}
                      />
                    ) : (
                      <div className="w-10 h-10 rounded-full bg-blue-500 flex items-center justify-center text-white font-bold text-sm mb-1">
                        {getInitials(onlineClass.user?.name)}
                      </div>
                    )}
                    <div className="text-sm text-gray-900 font-medium">
                      {onlineClass.user?.name || "N/A"}
                    </div>
                  </div>
                </td>
                {/* Subject */}
                ...
                <td className="px-4 py-4">
                  <div className="text-sm font-semibold text-gray-900">
                    {onlineClass.description || "N/A"}
                  </div>
                  <div className="text-xs text-gray-500 line-clamp-2">
                    {onlineClass.info.text || "No description"}
                  </div>
                </td>
                ...
                <td className="whitespace-nowrap px-4 py-4 text-sm text-gray-700">
                  <div className="font-medium">
                    {onlineClass?.date ? ((onlineClass.date)) : "N/A"}
                  </div>
                  <div className="text-xs text-gray-500">
                    {onlineClass.time || "N/A"}
                  </div>
                </td>

                {/* Info (Duration & Platform) */}
                <td className="whitespace-nowrap px-4 py-4 text-xs text-gray-700">
                  <div>Total Students: {onlineClass?.students?.length || "N/A"}</div>
