...
const HouseCreate = ({
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      console.log('formData', result?.results?.files?.[0]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (resp.data.success) {
        ...
        setSuccess(resp?.data?.results?.message || "House added successfully");
        ...
        setError(resp?.data?.results?.message || "Failed to add House");
        setSubmitted(false);
      }
    } catch (err) {
      console.error("House  error:", err);
      setError(err.message || "Something went wrong while adding House");
      setSubmitted(false);
    }
  };
